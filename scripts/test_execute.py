...
class TestJsonHelpers:
    ...
    def test_save_ensures_ascii_false(self, tmp_path):
        p = tmp_path / "test.json"
        ex.StepExecutor._write_json(p, {"korean": "test"})
        raw = p.read_text()
        assert "korean" in raw
        assert "\\u" not in raw
...
class TestLoadGuardrails:
    ...
    def test_empty_project(self, tmp_path):
        with patch.object(ex, "ROOT", tmp_path):
            # static-like behavior that doesn't need an executor, so use a temporary instance
...
class TestInvokeClaude:
    ...
    def test_timeout_is_900(self, executor):
...
# ---------------------------------------------------------------------------
# _check_blockers (= formerly main() error/blocked check)
# ---------------------------------------------------------------------------
